...
// Gold pairs confirmed from Kraken API
const GOLD_PAIRS = [
  'XAUTUSD',   // Tether Gold / USD
  'PAXGUSD',   // PAX Gold / USD
  'XAUTUSDT'   // Tether Gold / USDT
];
...
async function collectGold(pair) {
  console.log(`\n🔵 ${pair}`);
...
async function main() {
  console.log('\n' + '═'.repeat(60));
  console.log('🥇 GOLD DATA COLLECTOR');
  console.log('═'.repeat(60));
  console.log(`Pairs: ${GOLD_PAIRS.join(', ')}`);
  ...
  for (const pair of GOLD_PAIRS) {
    await collectGold(pair);
  }
  
  console.log(`\n🎉 Collection complete — ${totalNewCandles} new candles collected`);
  
  const summary = db.exec(`SELECT pair, interval, COUNT(*) as count FROM candles WHERE pair IN ('XAUTUSD', 'PAXGUSD', 'XAUTUSDT') GROUP BY pair, interval ORDER BY pair, interval`);
  if (summary.length) {
    console.log('\n📊 GOLD DATA SUMMARY');
    console.log('═'.repeat(60));
