...
async function test(target, stop, maxVol) {
  ...
  const result = db.exec(`
    ...
    FROM candles WHERE pair = 'BTC/USD' AND interval = '1H'
  ...
  for (let i = 20; i < candles.length; i++) {
    ...
    if (!position && pctFromCentre <= -0.5) {
      ...
      if (price >= position.target || price <= position.stop || hold >= 12) {
        capital *= (1 + pnl / 100);
...
async function run() {
  console.log('GRID 1H PARAMETER OPTIMISATION');
  ...
  for (const [t, s, v] of variants) {
    ...
    console.log(`  ${t}%    | ${s}%  | ${v}%     | ${String(r.total).padEnd(6)} | ${String(r.wr).padEnd(5)} | ${r.ret >= 0 ? '+' : ''}${r.ret}%  ${pass}`);
  }
}
